...
RH_RF22 radio_driver;

// Class to manage message delivery and receipt, using the driver declared above
RHRouter radio_manager(radio_driver, CLIENT_ADDRESS);
...
void test_routes()
{
  radio_manager.clearRoutingTable();
//  manager.printRoutingTable();
  radio_manager.addRouteTo(1, 101);
  radio_manager.addRouteTo(2, 102);
  radio_manager.addRouteTo(3, 103);
  RHRouter::RoutingTableEntry* e;
  e = radio_manager.getRouteTo(0);
  if (e) // Should fail
    Serial.println("getRouteTo 0 failed");
    
  e = radio_manager.getRouteTo(1);
  if (!e)
  ...
  if (!radio_manager.deleteRouteTo(1))
...
void test_tx()
{
  radio_manager.addRouteTo(SERVER_ADDRESS, ROUTER_ADDRESS);
  uint8_t errorcode;
  errorcode = radio_manager.sendtoWait(data, sizeof(data), 100); // Should fail with no route
  if (errorcode != RH_ROUTER_ERROR_NO_ROUTE)
    Serial.println("sendtoWait 1 failed");
  errorcode = radio_manager.sendtoWait(data, 255, 10); // Should fail too big
  if (errorcode != RH_ROUTER_ERROR_INVALID_LENGTH)
    Serial.println("sendtoWait 2 failed");   
  errorcode = radio_manager.sendtoWait(data, sizeof(data), SERVER_ADDRESS); // Should fail after timeouts to 110
