...
RHRouter radio_manager(radio_driver, SERVER3_ADDRESS);
...
void setup() 
{
  ...
  radio_manager.addRouteTo(CLIENT_ADDRESS, CLIENT_ADDRESS);  
  radio_manager.addRouteTo(SERVER2_ADDRESS, SERVER2_ADDRESS);
  radio_manager.addRouteTo(SERVER3_ADDRESS, SERVER2_ADDRESS);
}
...
void loop()
{
  ...
  uint8_t from;
  if (radio_manager.recvfromAck(buf, &len, &from))
  {
    ...
    if (radio_manager.sendtoWait(data, sizeof(data), from) != RH_ROUTER_ERROR_NONE)
