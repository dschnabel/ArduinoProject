...
void setup() 
{
  ...
  radio_manager.addRouteTo(SERVER1_ADDRESS, SERVER1_ADDRESS);  
  radio_manager.addRouteTo(SERVER2_ADDRESS, SERVER2_ADDRESS);
  radio_manager.addRouteTo(SERVER3_ADDRESS, SERVER3_ADDRESS);
}
...
void loop()
{
  ...
  if (radio_manager.sendtoWait(data, sizeof(data), SERVER3_ADDRESS) == RH_ROUTER_ERROR_NONE)
  {
    // It has been reliably delivered to the next node.
    // Now wait for a reply from the ultimate server
    uint8_t len = sizeof(buf);
    uint8_t from;    
    if (radio_manager.recvfromAckTimeout(buf, &len, 3000, &from))
    {
