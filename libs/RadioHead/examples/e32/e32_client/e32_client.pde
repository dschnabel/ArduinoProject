...
RH_E32  radio_driver(&mySerial, 4, 5, 8);
...
void loop() 
{
  ...
  radio_driver.send(data, sizeof(data));
  ...
  if (radio_driver.waitAvailableTimeout(10000)) // At 1kbps, reply can take a long time
  { 
    // Should be a reply message for us now   
    if (radio_driver.recv(buf, &len))
    {
