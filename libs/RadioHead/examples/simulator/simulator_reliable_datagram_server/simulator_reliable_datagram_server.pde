...
RHReliableDatagram radio_manager(radio_driver, SERVER_ADDRESS);
...
void setup() 
{
  ...
  // Defaults after init are 434.0MHz, 0.05MHz AFC pull-in, modulation FSK_Rb2_4Fd36
    radio_manager.setRetries(0); // Client will ping us if no ack received
...
void loop()
{
  ...
  if (radio_manager.recvfromAck(buf, &len, &from))
  {
      ...
      if (!radio_manager.sendtoWait(data, sizeof(data), from))
